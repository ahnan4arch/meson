...
class Python3Module(ExtensionModule):
    ...
    def get_version(self, state, args, kwargs):
        if args or kwargs:
            raise mesonlib.MesonException('get_version() takes no arguments.')
        return ModuleReturnValue(sysconfig.get_python_version(), [])

    def get_path(self, state, args, kwargs):
        if len(args) != 1:
            raise mesonlib.MesonException('get_path() requires passing the name of path to get.')
        if kwargs:
            raise mesonlib.MesonException('get_path() does not accept keywords.')
        path_name = args[0]
